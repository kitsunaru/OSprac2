...
class RandMMU(MMU):
    ...
    def read_memory(self, page_number):
        ...
        if page_number not in self.rand_mem_table:
            ...
            if self.rand_mem_table[0] in self.dirty_arr:
                self.write_count += 1
                self.dirty_arr.remove(self.rand_mem_table[0])
            if self.dbg:
                print("--------------------------------")
                print("######")
                print(f"Evict victim: page number: {self.rand_mem_table[0]}")
                print("######")
            self.rand_mem_table.pop(0)
    ...
    def write_memory(self, page_number):
        ...
        if page_number not in self.rand_mem_table:
            ...
            if self.rand_mem_table[0] in self.dirty_arr:
                ...
                self.write_count += 1
                if self.dbg:
                    print(f"Disk Write: {self.rand_mem_table[0]}")
                    print(f"stuff in memory: {self.rand_mem_table}")

            if self.dbg:
                if self.rand_mem_table[0] is not None:
                    print(f"Write: {self.rand_mem_table[0]}")
                print(f"Evict victim: page number: {self.rand_mem_table[0]}")
            self.rand_mem_table.pop(0)
            self.rand_mem_table.append(page_number)
